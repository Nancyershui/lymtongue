...
VMAMBA_DIR = REPO_ROOT / "Baseline" / "VMamba"
...
def build_vmamba_model(num_classes, img_size):
    if str(VMAMBA_DIR) not in sys.path:
        sys.path.insert(0, str(VMAMBA_DIR))
    try:
        from vmamba import VSSM
    except ImportError as exc:
        raise ImportError(
            "VMamba baseline requires Baseline/VMamba/vmamba.py or an installed vmamba module."
        ) from exc
