...
save_path = "checkpoints/densenet121_coordmamba_only.pth"
num_classes = 2
img_size = 224
batch_size = 16
num_epochs = 30
...
class DenseNet121_CoordMamba_Only(nn.Module):
    def __init__(
        self,
        num_classes=2,
        d_model=1024,
        ...
        print(">> [Ablation] 使用 DenseNet121 + Coord-Mamba only")
        print(">> [Backbone] 不修改 transition3，输出保持原始 DenseNet121 尺度（通常为 7x7）")
        print(f">> [Backbone] token 数 = {self.num_tokens}")
    ...
    def forward(self, x):
        ...
        return out
model = DenseNet121_CoordMamba_Only(
...
best_auc = 0.0
print(f"\nStart Training DenseNet121 + Coord-Mamba only [Seed={SEED}]...")
